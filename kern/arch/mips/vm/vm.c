            ...
            if((coremap[i].busy == false) && (coremap[i].page_state == PS_FREE || coremap[i].page_state == PS_CLEAN || coremap[i].page_state == PS_DIRTY)) {
                if(coremap[i].recent == false) {
                    ...
                    for(unsigned j = i; j < i + npages; j++) {
                        ...
                        if(coremap[j].pte != NULL && lock_do_i_hold(coremap[j].pte->p_lock)) {
                            available = false;
                            break;
                        }
                    }
...
vm_fault(int faulttype, vaddr_t faultaddress)
{
    ...
    if(pte->on_disk == true || pte->paddr == 0) {
        ...
        if(pte->on_disk == true) {
            ...
            KASSERT(bitmap_isset(swapmap,pte->swap_index));
...
alloc_upages(struct page_table_entry *pte)
{
    ...
    while(true) {
        ...
        if(swap_enable == false) {
            ...
            if(coremap[i].pte != NULL && lock_do_i_hold(coremap[i].pte->p_lock)) {
                i++;
                continue;
            }
            if(coremap[i].recent == false) {
